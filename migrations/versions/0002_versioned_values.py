...
def _version_columns(batch, table: str) -> None:
    batch.add_column(
        sa.Column("valid_from", sa.Date(), nullable=False, server_default=MIGRATION_VALID_FROM)
    )
...
def _upgrade_versioned_existing() -> None:
    ...
    op.create_index(
        ...
        "schedule_tasks",
        ["company_id", "object_id", "position_no", "front"],
        unique=True,
        postgresql_where=sa.text("valid_to IS NULL"),
        sqlite_where=sa.text("valid_to IS NULL"),
    )
...
def _create_cost_tables() -> None:
    ...
    op.bulk_insert(
        article,
        [
            {"id": uuid4(), "code": code, "category": category, "name": name, "sort_order": i}
            for i, (code, category, name) in enumerate(COST_ARTICLES, 1)
        ],
    )
    op.create_table(
        "cost_entries",
        ...
        sa.Column("work_item_id", sa.Uuid(), sa.ForeignKey("work_items.id")),
        sa.Column("article_code", sa.Text(), sa.ForeignKey("cost_articles.code"), nullable=False),
        sa.Column("quantity", sa.Numeric(18, 4)),
        sa.Column("unit", sa.Text()),
        sa.Column("price", sa.Numeric(18, 4)),
        sa.Column("amount", sa.Numeric(18, 2)),
        sa.Column("amount_type", sa.Text(), nullable=False, server_default="fixed"),
        sa.Column("rate_value", sa.Numeric(9, 6)),
        sa.Column("vat_mode", sa.Text(), nullable=False, server_default="unknown"),
        sa.Column("vat_rate", sa.Numeric(9, 6)),
        sa.Column("source_id", sa.Uuid(), sa.ForeignKey("value_sources.id"), nullable=False),
        sa.Column("valid_from", sa.Date(), nullable=False),
        sa.Column("valid_to", sa.Date()),
        sa.Column("superseded_by", sa.Uuid(), sa.ForeignKey("cost_entries.id")),
...
def _create_scenario_tables() -> None:
    op.create_table(
        ...
        sa.Column("valid_from", sa.Date(), nullable=False),
        sa.Column("valid_to", sa.Date()),
        sa.Column("superseded_by", sa.Uuid(), sa.ForeignKey("scenarios.id")),
        sa.Column("replace_reason", sa.Text()),
    )
    ...
    op.create_table(
        ...
        sa.Column("param_value", sa.Numeric(18, 6), nullable=False),
        sa.Column("created_by", sa.Text(), nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint(
        ...
        sa.CheckConstraint(
            "scope IN ('all','category','cost_item')", name="ck_scenario_param_scope"
        ),
...
def upgrade() -> None:
    ...
    if op.get_bind().dialect.name == "postgresql":
        for table in ("cost_entries", "scenarios", "scenario_params", "cost_articles"):
            op.execute(
                f"CREATE TRIGGER trg_{table}_immutable BEFORE UPDATE OR DELETE ON {table} FOR EACH ROW EXECUTE FUNCTION construction_os_guard_immutable();"
            )
...
def downgrade() -> None:
    ...
    op.drop_index("uq_schedule_task_current", table_name="schedule_tasks")
    op.drop_index("uq_object_current", table_name="objects")
    op.drop_index("uq_contract_current", table_name="contracts")
    naming = {"uq": "uq_%(table_name)s_%(column_0_name)s"}
    for table in ("schedule_tasks", "objects", "contracts"):
        with op.batch_alter_table(
            ...
            if table == "contracts":
                batch.drop_column("award_reduction_factor")
