...
from .models import (
    CompanyRow, ContractRow, CostArticleRow, CostEntryRow, DocumentRow, ObjectRow,
    ReferenceRateRow, ScenarioParamRow, ScenarioRow, ScheduleNoteRow, ScheduleTaskRow,
    ValueConfirmationRow, ValueRefRow, ValueSourceRow, WorkItemRow,
)
...
class BaseRepository:
    ...
    def _ensure_unique_active(self, company_id: UUID, values: dict[str, Any], exclude_id=None) -> None:
        ...
        if self.session.scalar(query) is not None:
            raise DuplicateActiveVersionError(f"{self.model.__tablename__}: duplicate active version {key_values}")
    ...
    def get_on_date(self, company_id: UUID, row_id: UUID, on_date: date):
        query = select(self.model).where(
            self.model.id == row_id, self.model.valid_from <= on_date,
    ...
    def supersede(self, company_id: UUID, old_id: UUID, new_values: dict[str, Any], reason: str, actor: str, effective_on: date):
        ...
        values = {column.name: getattr(old, column.name) for column in self.model.__table__.columns
        ...
        changed = {field: {"old": str(getattr(old, field)), "new": str(value)}
                   for field, value in new_values.items() if str(getattr(old, field)) != str(value)}
        self.session.add(ValueConfirmationRow(
            company_id=company_id, entity_name=self.model.__tablename__, entity_id=old.id, action="replaced",
...
class CompanyRepository(BaseRepository):
    ...
    def add(self, company_id: UUID, **values):
        row = CompanyRow(id=company_id, **values); self.session.add(row); self.session.flush(); return row
...
class DocumentRepository(BaseRepository): model = DocumentRow
class ValueSourceRepository(BaseRepository): model = ValueSourceRow
class ValueRefRepository(BaseRepository): model = ValueRefRow
...
class ContractRepository(BaseRepository):
    ...
    def _business_values(self, values: dict[str, Any]) -> dict[str, Any]:
        return {"number": values.get("number")} if values.get("number") is not None else {"number": None, "signed_on": values.get("signed_on")}
    def _validate_business_key(self, values: dict[str, Any]) -> None:
        if values.get("number") is None and values.get("signed_on") is None:
            raise InvalidBusinessKeyError("договор без номера и без даты: заполните хотя бы одно поле")
...
class ScheduleNoteRepository(BaseRepository): model = ScheduleNoteRow
class ValueConfirmationRepository(BaseRepository): model = ValueConfirmationRow
...
class WorkItemRepository(BaseRepository):
    ...
    def list_current(self, company_id: UUID, object_id: UUID | None = None, **filters):
        query = select(WorkItemRow).where(WorkItemRow.company_id == company_id, WorkItemRow.valid_to.is_(None))
...
class CostArticleRepository(BaseRepository):
    ...
    tenant_scoped = False
class CostEntryRepository(BaseRepository): model = CostEntryRow
class ScenarioRepository(BaseRepository):
    model = ScenarioRow
    business_key = ("object_id", "name")
class ScenarioParamRepository(BaseRepository): model = ScenarioParamRow

TENANT_REPOSITORIES = (
    DocumentRepository, ValueSourceRepository, ValueRefRepository, ContractRepository, ObjectRepository,
    WorkItemRepository, ScheduleTaskRepository, ScheduleNoteRepository, ValueConfirmationRepository,
    CostEntryRepository, ScenarioRepository, ScenarioParamRepository,
)
ALL_REPOSITORIES = (CompanyRepository, ReferenceRateRepository, CostArticleRepository, *TENANT_REPOSITORIES)
