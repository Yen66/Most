...
class EconomicsReport:
    ...
    max_reduction: ThresholdResult


def find_active_object(
    session, company_name: str, object_name: str
) -> tuple[CompanyRow, ObjectRow]:
    ...
    if len(rows) != 1:
        raise LookupError(
            f"ожидалась одна активная версия объекта {object_name!r}, найдено: {len(rows)}"
        )
    return rows[0]


def load_economics_report(
    session, company_name: str, object_name: str, on_date: date
) -> EconomicsReport:
    company, obj = find_active_object(session, company_name, object_name)
    ...
    if gross is None:
        raise LookupError("у объекта нет актуальной выручки")
    ...
    profit = None
    be = ThresholdResult(None, "нет данных")
    reduction = ThresholdResult(None, "нет данных")
    if costs is not None:
        tax = get_rate(RateType.PROFIT_TAX, on_date).value
        profit = calculate_profit(
            revenue.net, costs.production, costs.financial_fixed, costs.financial_share, tax
        )
        be = breakeven_net(costs.production, costs.financial_fixed, costs.financial_share)
        reduction = max_price_reduction(revenue.net, be.value)
