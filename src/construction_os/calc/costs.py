...
def summarize_costs(entries: list[CostEntry], articles: dict[str, CostArticle], expected_article_codes: set[str] | None = None, default_vat_rate: Decimal | None = None) -> CostSummary | None:
    if not entries:
        return None
    by_article={}; by_category={}; financial_fixed=Decimal("0"); financial_share=Decimal("0")
    unknown_count=0; unknown_amount=Decimal("0"); warnings=[]; present=set()
    for entry in entries:
        article=articles[entry.article_code]; present.add(article.code)
        ...
        if unknown:
            unknown_count+=1; unknown_amount+=amount
        ...
        if article.category=="financial": financial_fixed+=amount
        if article.category=="other": warnings.append(f"{article.code}: статья не классифицирована, уточните категорию")
    production=sum((v for k,v in by_category.items() if k!="financial"),Decimal("0"))
    if unknown_count:
        warnings.append(f"{unknown_count} записей на сумму {money(unknown_amount)} ₽ имеют неопределённый режим НДС. Результат может быть занижен или завышен")
    missing=tuple(sorted((expected_article_codes or set())-present))
    return CostSummary({k:money(v) for k,v in by_article.items()},{k:money(v) for k,v in by_category.items()},money(production),money(financial_fixed),financial_share,unknown_count,money(unknown_amount),tuple(warnings),missing,not missing and unknown_count==0)
