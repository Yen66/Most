...
def persist_costs(
    ...
    for row in parsed.rows:
        if row.object_name not in objects:
            raise CostImportError(f"строка {row.row_no}: объект не найден: {row.object_name}")
    path = Path(source_path)
    document = _create_document(session, company.id, path, "costs", parsed.sha256)
    created = 1
    first = None
    effective = imported_on or date.today()
    for row in parsed.rows:
        obj = objects[row.object_name]
        first = first or obj.id
        work_id = None
        if row.work_position_no is not None:
            work = session.scalar(
                select(WorkItemRow).where(
                    WorkItemRow.company_id == company.id,
                    WorkItemRow.object_id == obj.id,
                    WorkItemRow.position_no == row.work_position_no,
                    WorkItemRow.valid_to.is_(None),
                )
            )
            if work is None:
                raise CostImportError(
                    f"строка {row.row_no}: позиция работ не найдена: {row.work_position_no}"
                )
            work_id = work.id
        ...
        session.add(entry)
        session.flush()
        field = "rate_value" if row.amount_type == "share_of_revenue" else "amount"
        column = "I" if field == "rate_value" else "G"
        cell_source = ValueSourceRow(
            company_id=company.id,
            source_type=SourceType.DOCUMENT.value,
            document_id=document.id,
            sheet=parsed.sheet_name,
            cell_or_range=f"{column}{row.row_no}",
            row_no=row.row_no,
            confidence="exact",
        )
        ...
        created += 4
    session.flush()
    return PersistResult(document.id, company.id, first, created, False)
