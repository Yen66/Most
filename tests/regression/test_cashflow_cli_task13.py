...
def setup(tmp_path, monkeypatch):
    ...
    with Session(engine) as session:
        ...
        contract = ContractRepository(session).add(
            company.id, contract_type="government", number="DEMO",
            signed_on=date(2026, 8, 1), price_is_final=True,
            valid_from=date(2026, 8, 1),
        )
        create_act(
            session, company.id, contract, "CF1", Decimal("1000000"),
            date(2026, 9, 1), signed_on=date(2026, 9, 29), via_eis=True,
        )
        session.commit()
    return engine
...
def test_CLI_CF1_and_idempotent_build(tmp_path, monkeypatch, capsys):
    ...
    assert main(cmd("add", "--date", "2026-09-15", "--direction", "out",
                    "--amount", "600000", "--category", "MAT",
                    "--contract-number", "DEMO")) == 0
    assert main(cmd("add", "--date", "2026-09-30", "--direction", "out",
                    "--amount", "200000", "--category", "LAB",
                    "--contract-number", "DEMO")) == 0
...
def test_CLI_unknown_category(tmp_path, monkeypatch, capsys):
    setup(tmp_path, monkeypatch)
    assert main(cmd("add", "--date", "2026-09-15", "--direction", "in",
                    "--amount", "1", "--category", "unknown")) == 2
...
def test_CLI_plan_override(tmp_path, monkeypatch, capsys):
    setup(tmp_path, monkeypatch)
    assert main(cmd("add", "--date", "2026-09-15", "--direction", "out",
                    "--amount", "100", "--category", "MAT", "--plan")) == 0
    capsys.readouterr()
...
def test_CLI_future_plan_separate(tmp_path, monkeypatch, capsys):
    setup(tmp_path, monkeypatch)
    assert main(cmd("add", "--date", "2026-09-15", "--direction", "out",
                    "--amount", "100", "--category", "MAT")) == 0
    assert main(cmd("add", "--date", "2026-11-01", "--direction", "in",
                    "--amount", "1000", "--category", "advance", "--plan")) == 0
    capsys.readouterr()
    assert main(cmd("report", "--as-of", "2026-10-31", "--rate", "0.14")) == 0
    output = capsys.readouterr().out
