...
def seed(session, suffix="A"):
    ...
    if (
        session.scalar(
            select(WorkCalendarRow.id).where(WorkCalendarRow.cal_date == T("2026-09-29"))
        )
        is None
    ):
        session.add_all(WorkCalendarRow(**row) for row in iter_calendar_days(2026))
    for code in ("MAT", "LAB", "SUB"):
        if session.scalar(select(CostArticleRow.id).where(CostArticleRow.code == code)) is None:
...
def test_manual_duplicate_allowed_and_provenance(sqlite_session):
    ...
    rows = list(
        sqlite_session.scalars(select(CashFlowRow).where(CashFlowRow.source_kind.is_(None)))
    )
    assert len(rows) == 2
    for row in rows:
        assert (
            sqlite_session.scalar(
                select(func.count())
                .select_from(ValueRefRow)
                .where(ValueRefRow.entity_id == row.id, ValueRefRow.entity_name == "cash_flows")
            )
            == 5
        )
...
def test_postgres_cashflow_immutable(db_session):
    ...
    with pytest.raises(Exception, match="cash_flows immutable"), db_session.begin_nested():
        db_session.execute(
            text("UPDATE cash_flows SET amount=11 WHERE id=:id"),
            {
                "id": str(row.id),
            },
        )
