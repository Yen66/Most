...
def test_report_signing_status_and_active_contract_cap(tmp_path, monkeypatch, capsys):
    ...
    with Session(engine) as session:
        ...
        create_act(
            session, company.id, contract, "SIGNED", Decimal("1000000"),
            date(2026, 9, 1), signed_on=date(2026, 9, 29), via_eis=True,
        )
        create_act(
            session, company.id, contract, "PENDING", Decimal("100"),
            date(2026, 9, 1), via_eis=True,
        )
        repo.supersede(
            company.id, contract.id,
            {"penalty_cap_pct": Decimal("0.005")},
            "contract cap", "test", date(2026, 10, 1),
        )
        session.commit()
    assert main([
        "acts", "report", "--company", "Report Co", "--as-of", "2026-10-28",
    ]) == 0
...
def test_report_refusal_reset_notice(tmp_path, monkeypatch, capsys):
    ...
    with Session(engine) as session:
        ...
        change_act_status(
            session, company.id, first,
            refusal_on=date(2026, 9, 20), refusal_reason="Замечания",
        )
        create_act(
            session, company.id, contract, "A1", Decimal("100"),
            date(2026, 9, 25), via_eis=True,
        )
        session.commit()
    assert main([
        "acts", "report", "--company", "Refusal Co", "--as-of", "2026-10-01",
    ]) == 0
